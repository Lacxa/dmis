...
class Doctor extends CI_Controller {
  ...
  {
    $this->form_validation->set_rules('record', 'Patient', 'trim|required|numeric');
    $this->form_validation->set_rules('visit', 'Visit', 'trim|required|numeric');
    ...
    else
    {
      ...
      $patient_visit_id = $this->security->xss_clean($this->input->post('visit'));
        ...
        { 
          ...
          if(empty($visit_data))
          {
            echo json_encode(array("status" => FALSE, 'data' => '<span class="text-danger"> Invalid action: not a fresh client </span>'));
            exit();
          }
          else
          {
            ...
            echo json_encode(array("status" => TRUE, 'data' => 'Success', 'redirect' => base_url('doctor/session-patients/'.$patient_instance)));
            exit();
          }
        }
    }
            ...
            else
            {
              ...
              { 
                if($stock_token != '10000001')
                {
                  ...
                  {
                    echo json_encode(array("status" => FALSE, 'data' => '<span class="text-danger">Oops!, we are running out of stock</span>'));
                    exit();                    
                  }
                }
                ...
                else
                {
                  ...
                  {
                    echo json_encode(array("status" => FALSE, 'data' => '<span class="text-danger">Oops!, arleady exist in a list</span>'));
                    exit();
                  }
                  else
                  {
  ...
  { 
    if($this->input->server('REQUEST_METHOD') === 'POST')
    {
      $this->form_validation->set_rules('query', 'Keyword Number', 'trim|required');
